...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  switch (type) {
    ...
    case TYPE_B:  immB(); break;//src1R(); src2R();
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 0000011" , lb     , I ,R(rd) = Mr(src1 + imm, 1));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 0110011" , div    , RE,R(rd) = (int32_t)src1 / (int32_t)src2);
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 0110011" , sra    , RE,R(rd) = (int32_t)src1 >> (src2 & 0x1F));
  INSTPAT("0000000 ????? ????? 101 ????? 0110011" , srl    , RE,R(rd) = src1 >> (src2 & 0x1F));
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 0110011" , mulh   , RE,R(rd) = ((int64_t)((int32_t)src1) * (int64_t)((int32_t)src2)) >> 32);
  INSTPAT("0000001 ????? ????? 011 ????? 0110011" , mulhu ,  RE,R(rd) = ((uint64_t)src1 * (uint64_t)src2) >> 32);
  INSTPAT("0000001 ????? ????? 111 ????? 0110011" , remu   , RE,R(rd) = src1 % src2);
  INSTPAT("0000001 ????? ????? 101 ????? 0110011" , divu   , RE,R(rd) = src1 / src2);
...
int isa_exec_once(Decode *s) {
  ...
  IFDEF(CONFIG_IRTRACE_N_Y, iringbuf(s));
