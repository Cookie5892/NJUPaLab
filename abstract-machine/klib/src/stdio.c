...
// 提取公共的格式化逻辑
static int format_core(char *out, size_t n, const char *fmt, va_list ap) {
  char *dst = out;
  const char *p = fmt;
  if (n == 0 ) return 0;
...
int printf(const char *fmt, ...) {
  ...
  va_list ap;
  va_start(ap, fmt);
  int result = format_core(buffer, sizeof(buffer), fmt, ap);
  va_end(ap);
...
int vsprintf(char *out, const char *fmt, va_list ap) {
  // 假设输出缓冲区足够大
  return format_core(out, SIZE_MAX, fmt, ap);
}
...
static int vsnprintf_core(char *out, size_t n, const char *fmt, va_list ap) {
  return format_core(out, n, fmt, ap);
}
